...
//문제 : 나무 섭지
//url : https://softeer.ai/app/assessment/index.html?xid=81293&xsrfToken=Z6V064ekbADA5VO4yPxuEjVhTNzSWaJx&testType=practice
class Solution_012 {
...
class Solution_014 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < N; i++) {
            queue.add(Integer.parseInt(sm.nextToken()));
        }
    ...
    public static void searchAll(int n, int sum, int cnt) {
        for (int i = n + 1; i < ans.size(); i++) {
            int ssum = sum + ans.get(i);
            test.add(ssum);
            searchAll(i, ssum, cnt + 1);
        }
    }
}
